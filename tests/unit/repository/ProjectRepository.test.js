...
describe('Repository', () => {
  ...
  describe('getUserProposals', () => {
    it('should resolve with response from sequelize.query when user.operation is "projetos"', async () => {
      ...
      expect(sequelize.query).toHaveBeenCalledWith(`SELECT p."projectId", p.name, p."expectedResult", p.status, p."createdAt", s.name AS "Subject", cu."fullName" FROM "Project" p LEFT JOIN "Subject" s on p."subjectId" = s."subjectId" LEFT JOIN "User" cu on p."userId" = cu."userId" WHERE not(p.deleted) ORDER BY p."projectId" DESC`);
    });
    ...
    it('should resolve with response from sequelize.query when user.operation is not "projetos" or "projetos-disciplina"', async () => {
      ...
      expect(result).toBe(expectedResult);
      expect(sequelize.query).toHaveBeenCalledWith(`SELECT p."projectId", p.name, p."expectedResult", p.status, p."createdAt", s.name AS "Subject", cu."fullName" FROM "Project" p LEFT JOIN "Subject" s on p."subjectId" = s."subjectId" LEFT JOIN "User" cu on p."userId" = cu."userId" WHERE not(p.deleted) and p."userId" = 1 ORDER BY p."projectId" DESC`);
    });
