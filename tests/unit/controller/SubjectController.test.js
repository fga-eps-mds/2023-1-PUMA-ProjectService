...
describe("subject controller", () => {
  ...
  describe("getKeywords()", () => {
    it("should resolve with response when successful", async () => {
      ...
      expect(
        keywordRepository.getKeywordAvailbleToSubject
  ...
  describe("getKnowledgeAreas", () => {
    it("should return an array of knowledge areas with their respective subareas", async () => {
      ...
      knowledgeAreaRepository.getKnowledgeAreas = jest
        .fn()
        .mockResolvedValue(knowledgeAreas);
    ...
    it("should throw an error if any of the promises fails", async () => {
      ...
      await expect(subjectController.getKnowledgeAreas()).rejects.toThrow(
        errorMessage
      );
    });
  });
  ...
  describe("getSubareas", () => {
    ...
    it("should throw an error if any of the promises fails", async () => {
      ...
      subareaRepository.getSubareas = jest
        .fn()
        .mockRejectedValue(new Error(errorMessage));

      await expect(subjectController.getSubareas()).rejects.toThrow(
        errorMessage
      );
    });
  });
  ...
  describe("deleteSubject", () => {
    it("should delete the subject with the given ID", async () => {
      ...
      expect(subjectRepository.deleteSubject).toHaveBeenCalledWith(
        mockSubjectId
      );
    });
    ...
    it("should reject with an error if the repository call fails", async () => {
      ...
      await expect(
        subjectController.deleteSubject(mockSubjectId)
  ...
  describe("getProfessors", () => {
    ...
    it("should reject with an error if the repository call fails", async () => {
      ...
      await expect(subjectController.getProfessors()).rejects.toThrow(
        mockError
      );
    });
  });
  ...
  describe("getSubject", () => {
    ...
    it("should return subject, keywords, subareas, and professors", async () => {
      subjectRepository.getSubject.mockResolvedValue({
        subjectId: 1,
        name: "Math",
      });
      keywordRepository.getKeywordsOfSubject.mockResolvedValue([
        { keywordId: 1, name: "Algebra" },
      ]);
      subareaRepository.getSubareasOfSubject.mockResolvedValue([
        { subareaId: 1, name: "Mathematics" },
      ]);
      professorRepository.getProfessorsofSubject.mockResolvedValue([
        { regnumber: 1234, name: "John Doe" },
      ]);
    ...
    it("should reject with an error if any repository function throws an error", async () => {
      ...
      await expect(subjectController.getSubject(input)).rejects.toThrow(
        "Some error"
      );
    });
  });
  ...
  describe("updateSubject", () => {
    ...
    it("should update subject and related entities", async () => {
      ...
      keywordRepository.addKeyword.mockResolvedValue({
        keywordId: 1,
        keyword: "Algebra",
      });
    ...
    it("should reject with an error if any repository function throws an error", async () => {
      keywordRepository.removeKeywordsOfSubject.mockRejectedValue(
        new Error("Some error")
      );

      await expect(subjectController.updateSubject(input)).rejects.toThrow(
        "Some error"
      );
    });
  });
